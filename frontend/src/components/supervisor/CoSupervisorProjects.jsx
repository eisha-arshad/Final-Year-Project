...
import styles from "./CoSupervisorProjects.module.css";

/** Keep steps IDENTICAL to SupervisorProjects.jsx */
const STEPS = [
...
export default function CoSupervisorProjects() {
  ...
  const loadProjects = async () => {
    ...
    setStatus("");
    try {
      // Co-supervisor list endpoint (you used this earlier)
      const res = await api.get("/api/co-supervisor-projects/");
      setProjects(res.data || []);
    } catch {
      setStatus("Failed to load your co-supervised projects.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // progress (linked to submissions like SupervisorProjects)
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.title}>My Co-Supervised Projects</h2>
      ...
      {loading ? (
        ...
        <div className={styles.empty}>You have no co-supervised projects yet.</div>
      ) : (
                        ...
                        comments.map((c) => (
                          <div key={c.id || c.created_at} className={styles.commentItem}>
                            <div className={styles.commentMeta}>
                              <b>{c.author_name || "Co-Supervisor"}</b>
